...
traverseBFS(NodeArray &arr)
{
  ...
  for (; count < arr.size(); ++count) {
    nodeInfo *root = arr[count];
    if (!root->isUndefined() && !root->isUnprocessed()) {
      continue;
    }
#endif
    ...
    while (!q.empty()) {
      ...
      q.pop();
      if (!obj->isUndefined() && !obj->isUnprocessed()) {
        continue;
      }
      ...
      for (size_t index = 0; index < neighbors.size(); ++index) {
        ...
        if (obj->isNeighbor(*child) == 1) {
          child->setState(nodeInfo::UNPROCESSED);
        } else if (obj->isNeighbor(*child) == 2) {
          child->setState(nodeInfo::SKIP);
        }
        q.push(child);
      }
    }
